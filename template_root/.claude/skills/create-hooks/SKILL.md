...
2. Check COST_TIER: `${COST_TIER:-free}` - hooks are free (shell commands)
3. Warn on HIGH/CRITICAL anti-patterns (proceed with caution)
4. **Safety reminder**: Blocking hooks (PreToolUse, Stop) can disrupt workflows - test carefully
5. Log: `echo "[$(date -Iseconds)] [create-hooks] [CREATE] tier=${COST_TIER:-free} agent=none reason=\"hook_creation\"" >> /tmp/teaming-decisions.log`
</pre_flight>

<post_verification>
After creating hook:
1. Validate JSON: `jq . .claude/hooks.json`
2. Check script permissions: `ls -la hooks/*.sh` (if using script files)
3. Suggest testing: "Test with: `claude --debug` and trigger the hook event"
4. If blocking hook: Warn about testing in isolation first
</post_verification>
</teaming_integration>
