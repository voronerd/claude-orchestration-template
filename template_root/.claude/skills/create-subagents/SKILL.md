...
2. Check COST_TIER: `${COST_TIER:-free}` - subagents have cost implications (model selection)
3. Warn on HIGH/CRITICAL anti-patterns (proceed with caution)
4. Log: `echo "[$(date -Iseconds)] [create-subagents] [CREATE] tier=${COST_TIER:-free} agent=none reason=\"subagent_creation\"" >> /tmp/teaming-decisions.log`
</pre_flight>

<post_verification>
After creating subagent:
1. Validate YAML frontmatter: `head -10 .claude/agents/{name}.md`
2. Suggest testing: "Test with: `Use the {name} subagent to [example task]`"
3. Suggest audit: "Consider running `/subagent-auditor` from taches-cc-resources"
4. Cost awareness: If model=opus, warn about cost implications
</post_verification>
</teaming_integration>
