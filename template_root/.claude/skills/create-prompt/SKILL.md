---
name: create-prompt
description: Create prompts with integrated teaming checks for cost-conscious agent routing
argument-hint: [task description]
arguments:
  - name: prompt_id
    description: The prompt ID or task description (can also use $ARGUMENTS)
    required: false
  - name: task_file
    description: Path to task spec file
    required: false
  - name: skip_teaming
...
<quick_start>
To create a prompt quickly:
1. Run `/create-prompt <description>` with your task description
2. Answer the adaptive questionnaire (2-4 questions)
3. Select "Proceed" when ready
4. Choose to run immediately or save for later
</quick_start>
...
<title>Pre-Flight Teaming Checks</title>

<skip_check>
If `skip_teaming=true` is passed:
- Log bypass to `/tmp/teaming-decisions.log`
- Skip all teaming checks
- Add warning comment to generated prompt
- Proceed directly to step_1_intake_gate
</skip_check>
...
```bash
# Check for latest doctor report
cat /tmp/doctor-report-latest.json 2>/dev/null || echo '{"antiPatterns":[]}'
```

If doctor report contains active anti-patterns with severity HIGH or CRITICAL:
- Display warning to user
- Suggest remediation before proceeding
</doctor_check>

<scope_analysis>
Analyze Prompt Scope

Before creating the prompt, estimate scope:
- Read the task file if provided
- Count expected files (look for file lists, module structures)
- Identify if this is INFRASTRUCTURE (new modules, packages) vs FEATURE (modifications)

**Infrastructure indicators:**
- Creates new directories
- Adds new packages/modules
- Sets up new integrations
</scope_analysis>

<teaming_matrix>
Teaming Decision Matrix

| Scope | Files Planned | Required Teaming |
|-------|---------------|------------------|
| Feature | 1-3 | None - proceed to step_1 |
| Feature | 4+ | Suggest @local-orchestrator review |
| Infrastructure | Any | REQUIRE integration_plan field |
| Infrastructure | 4+ | REQUIRE @overseer pre-review |
</teaming_matrix>

<cost_check>
Cost Tier Check

Check environment for cost preference:
```bash
echo ${COST_TIER:-free}
```

If `COST_TIER=free`:
- Prefer @local-orchestrator over @overseer
- Prefer @local-coder over @general-purpose
- Only escalate to paid agents if local fails
</cost_check>

<integration_checkpoint>
Integration Checkpoint

For INFRASTRUCTURE prompts, require explicit integration plan:

Ask user:
> This appears to be an infrastructure prompt. Which file(s) will import/wire this new code?
> (This prevents orphaned infrastructure - code that passes tests but isn't integrated)

Store answer in prompt metadata.
</integration_checkpoint>

<teaming_logging>
Log Teaming Decision

After all checks pass:
1. Log teaming decision to `/tmp/teaming-decisions.log`
2. If infrastructure, note the integration checkpoint answer
3. Proceed to step_1_intake_gate
</teaming_logging>
...
**Infrastructure Prompt Addition:**
If this was identified as INFRASTRUCTURE in step_0, add a reminder about the integration checkpoint:
```xml
<integration_reminder>
Integration target(s): [files from integration checkpoint]
...
<success_criteria>
- Teaming checks completed (or skipped if skip_teaming=true)
- Doctor warnings displayed if applicable
- Integration checkpoint answered for infrastructure prompts
...
- **Teaming first**: Complete step_0_teaming_checks before intake. Check doctor report and scope.
- **Intake second**: Complete step_1_intake_gate before generating. Use AskUserQuestion for structured clarification.
...
- Log all teaming decisions to `/tmp/teaming-decisions.log`
</meta_instructions>
