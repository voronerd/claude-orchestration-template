...
**AT SESSION START**, check for anti-patterns:
```bash
DOCTOR_JSON=$(cat /tmp/doctor-report-latest.json 2>/dev/null) || DOCTOR_JSON='{"antiPatterns":[]}'
HIGH_PATTERNS=$(echo "$DOCTOR_JSON" | jq -r '.antiPatterns[] | select(.severity == "HIGH" or .severity == "CRITICAL") | "\(.name): \(.description)"' 2>/dev/null)
[ -n "$HIGH_PATTERNS" ] && echo "[TEAMING WARNING] Context rot or anti-patterns detected: $HIGH_PATTERNS"
```

### Agent Routing by Complexity
| Pipeline Type | Routing |
|---------------|---------|
| Single prompt (Do/Research) | Standard generation |
| Multi-stage pipeline (3+ prompts) | Suggest @overseer review before execution |
| Chained dependencies | Warn about execution cost |

### Cost Estimation Display
Before execution, show:
```
Estimated agent usage:
- Prompt generation: FREE (current session)
- Execution (N prompts): FREE (@lite-general) or PAID (if complex)
- Total paid API calls: ~X (if COST_TIER=paid)
```

### Integration Checkpoint
**For Do prompts**, ask:
> Where will this prompt's output be consumed? (file path or "standalone")

Add to generated prompts:
```xml
<integration_reminder>
Output consumer: {target}
After execution, verify integration at above location.
</integration_reminder>
```

### Logging
```bash
echo "[$(date -Iseconds)] [create-meta-prompts] [ROUTE] tier=${COST_TIER:-free} purpose={purpose} prompts={count}" >> /tmp/teaming-decisions.log
```
</teaming_integration>
...
Existing research/plans: !`find ./prompts -name "*-research.md" -o -name "*-plan.md" 2>/dev/null`
Next prompt number: !`ls -d ./prompts/*/ 2>/dev/null`
</context>
