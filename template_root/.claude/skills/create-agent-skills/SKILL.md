...
2. Check COST_TIER: `${COST_TIER:-free}` - complex skills may need review
3. Assess complexity: Simple (single file) vs Router (workflows/references)
4. Warn on HIGH/CRITICAL anti-patterns (proceed with caution)
5. Log: `echo "[$(date -Iseconds)] [create-agent-skills] [CREATE] tier=${COST_TIER:-free} agent=none reason=\"skill_creation\"" >> /tmp/teaming-decisions.log`
</pre_flight>

<post_verification>
After creating skill:
1. Validate YAML frontmatter: `head -10 .claude/skills/{name}/SKILL.md`
2. Check line count: `wc -l .claude/skills/{name}/SKILL.md` (should be <500)
3. Suggest audit: "Consider running `taches-cc-resources:skill-auditor` for structure validation"
4. If router pattern: Verify workflows/ and references/ directories exist
</post_verification>
</teaming_integration>
