...
2. Check COST_TIER: `${COST_TIER:-free}` - commands are free (prompt expansion)
3. Warn on HIGH/CRITICAL anti-patterns (proceed with caution)
4. Log: `echo "[$(date -Iseconds)] [create-slash-commands] [CREATE] tier=${COST_TIER:-free} agent=none reason=\"command_creation\"" >> /tmp/teaming-decisions.log`
</pre_flight>

<post_verification>
After creating command:
1. Validate YAML frontmatter: `head -5 .claude/commands/{name}.md`
2. Check XML structure: Verify `<objective>`, `<process>`, `<success_criteria>` tags present
3. Suggest testing: "Test with: `/{name} [args]` in a fresh conversation"
4. Suggest audit: "Consider running `taches-cc-resources:slash-command-auditor`"
</post_verification>
</teaming_integration>
