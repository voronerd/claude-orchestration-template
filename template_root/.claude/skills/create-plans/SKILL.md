...
<teaming_integration>
<reference>See: .claude/skills/create-prompt/references/teaming-middleware.md</reference>

<pre_flight>
### Teaming Check
1. Read doctor report: `cat /tmp/doctor-report-latest.json 2>/dev/null || echo '{}'`
2. Check COST_TIER: `${COST_TIER:-free}` - planning may trigger research agents (potentially paid)
3. Warn on HIGH/CRITICAL anti-patterns (proceed with caution)
4. Log: `echo "[$(date -Iseconds)] [create-plans] [CREATE] tier=${COST_TIER:-free} agent=none reason=\"plan_creation\"" >> /tmp/teaming-decisions.log`
</pre_flight>

<integration_checkpoint>
For infrastructure plans (new packages, modules, integrations):
> Where will plan outputs integrate? Which files will import/wire the new code?
Store answer in plan as `integration_targets:` to prevent orphaned infrastructure.
</integration_checkpoint>

<post_verification>
After creating plan:
1. Validate plan structure: Check for Objective, Tasks, Verification, Success criteria
2. Verify phase directory exists: `ls -la .planning/phases/{phase}/`
3. If research needed: Note that `/research:deep-dive` costs API tokens (COST_TIER=paid)
4. Integration check: Confirm integration_targets specified for infrastructure plans
</post_verification>
</teaming_integration>
